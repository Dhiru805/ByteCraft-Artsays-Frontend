...
const ContactUs = () => {
  ...
  const fetchSEOMetadata = async () => {
    try {
      const response = await axiosInstance.get("/api/contactus/getSEO");
      const meta = response.data?.data || {};

      setSeoData({
        metaTitle: meta.metaTitle || "Contact Us - Artsays",
        metaDescription:
          meta.metaDescription ||
          "Get in touch with Artsays. Contact our team for inquiries, support, or to commission unique artworks.",
        metaKeywords:
      ...
      console.error("Error fetching Contact Us SEO metadata:", error);
  ...
  return (
    <div className="w-full font-[poppins]">
        ...
        <script type="application/ld+json">
          {JSON.stringify({
            ...
            "itemListElement": [
              ...
              { "@type": "ListItem", "position": 2, "name": "Contact Us", "item": "https://artsays.in/contact-us" }
            ]
          })}
        </script>
      </Helmet>
      ...
      <nav aria-label="breadcrumb" className="max-w-[1440px] mx-auto px-4 md:px-8 flex items-center gap-2 text-sm text-gray-500 py-3">
        ...
        <span className="text-[#6F4D34] font-semibold" aria-current="page">Contact Us</span>
      </nav>
      <ContactUsContent />
