...
const Login = () => {
  ...
  const navigate = useNavigate();
  const { login, userType, status: userStatus } = useAuth(); // Renamed to userStatus
  ...
  useEffect(() => {
    ...
    if (token && storedUserType) {
      if ((storedUserType === 'Artist' || storedUserType === 'Seller') && storedStatus === 'Rejected') {
        setShowPopup(true);
  ...
  useEffect(() => {
    ...
    if ((userType === 'Artist' || userType === 'Seller') && userStatus === 'Rejected') {
      console.log('Setting showPopup to true for unverified user.');
      setShowPopup(true);
    } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      console.log('Submitting login request with:', { emailOrPhone: input });
      const res = await postAPI('/auth/login', {
        emailOrPhone: input,
        password,
      }, true);
      ...
      toast.success('Login Successful!');
      console.log('Login successful. Post-login state:', { userType, status });

      if (userType === 'Buyer') {
        navigate('/');
      } else if ((userType === 'Artist' || userType === 'Seller') && status === 'Rejected') {
        setShowPopup(true);
      } else {
        navigate(`/${userType.toLowerCase()}/dashboard`);
      }
