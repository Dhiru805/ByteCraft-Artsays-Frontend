...
import { Search, ChevronRight, Info, HelpCircle, ShieldCheck, FileText, Lock } from "lucide-react";
...
const LicensingPartnerContent = () => {
    ...
    useEffect(() => {
        const fetchPublishedPage = async () => {
            try {
                const res = await axiosInstance.get("/api/licensing/published");
                if (res.data.success && res.data.data) {
                    setPageData(res.data.data);
                }
            } catch (error) {
                console.error("Error fetching Licensing page:", error);
            } finally {
                setLoading(false);
            }
        };
                    ...
                    <aside className="w-full lg:w-[300px] shrink-0">
                        <div className="sticky top-6 space-y-4">
                            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                                <h2 className="text-xl font-bold text-gray-900 mb-1">Licensing Info</h2>
                                <nav className="flex items-center text-sm text-gray-500 mt-2">
                                    <a href="/" className="hover:text-[#6F4D34] transition-colors">Home</a>
                                    <ChevronRight size={14} className="mx-2" />
                                    <span className="text-[#6F4D34] font-medium">Licensing</span>
                                </nav>
                            </div>

                            <SidebarCard title="Why Licensing?" icon={ShieldCheck}>
                                <p className="text-sm text-gray-600 leading-relaxed">
                                    Licensing protects your intellectual property while allowing for ethical commercial use and distribution of creative works.
                                </p>
                            </SidebarCard>

                            <SidebarCard title="Legal Protection" icon={Lock}>
                                <p className="text-sm text-gray-600 leading-relaxed">
                                    Our partnership with Creative Rights Alliance ensures global legal standards for every transaction.
                                </p>
                            </SidebarCard>
                            ...
                            <SidebarCard title="Need Help?" icon={HelpCircle}>
                                <div className="space-y-3">
                                    ...
                                    <button className="w-full flex items-center justify-between p-3 rounded-xl bg-gray-50 hover:bg-[#6F4D34]/5 text-gray-700 hover:text-[#6F4D34] transition-all group">
                                        <span className="text-sm font-semibold">Licensing FAQ</span>
                                        <ChevronRight size={16} className="group-hover:translate-x-1 transition-transform" />
                                    </button>
                                </div>
                            </SidebarCard>
                            ...
                            <div className="bg-[#6F4D34] p-6 rounded-2xl shadow-xl text-white relative overflow-hidden group">
                                <div className="relative z-10">
                                    ...
                                    <p className="text-sm opacity-90 mb-4">Join our network of licensing excellence.</p>
                    ...
                    <main className="flex-grow space-y-3">
                        ...
                        <div className="space-y-3">
                            <div className="relative group">
                                <input
                                    type="text"
                                    placeholder="Search licensing topics..."
                                    value={searchTerm}
