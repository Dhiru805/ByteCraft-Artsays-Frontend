...
const [estimatedDeliveryDate, setEstimatedDeliveryDate] = useState("—");
  ...
  const CANCEL_ENDPOINTS = [
    (id) => `/api/buyer-order-list/cancel/${encodeURIComponent(id)}`,
  ];
  ...
  try {
    const res = await getAPI(
      `/api/buyer-order-list/${buyerId}/${orderIdFromUrl}`
    );
...
useEffect(() => {
  ...
  const fetchDeliveryEstimate = async () => {
    try {
      const productIds = order.items.map(
        (it) => (it.productId?._id || it.productId || it._id)?.toString()
      );
      ...
      if (!matched || !matched.estimatedDelivery) {
        console.log("No estimatedDelivery found");
        setEstimatedDeliveryDate("—");
        return;
      }

      const range = matched.estimatedDelivery; 
      console.log("Matched Delivery Range:", range);
  ...
  const unifiedItems = (order?.items || []).map((it) => {
    ...
    return {
      ...
      fullProduct,
      mainImage:
        (fullProduct && (fullProduct.mainImage || fullProduct.image)) ||
        it.image ||
        it.mainImage ||
        null,
      name:
        it.name ||
        (fullProduct && (fullProduct.productName || fullProduct.productName)) ||
        (it.productId && it.productId.name) ||
        "",
  ...
  const finalPrice = order?.totalAmount || order?.finalPrice || order?.amount || order?.total;
  const sellerName = order?.Artist?.name || order?.Artist?.id || order?.sellerName || order?.seller?.name;
  const buyer = order?.Buyer || order?.buyer || order?.BuyerDetails || {};
  const address =
    order?.BuyerSelectedAddress ||
  ...
  if (!order) {
    return (
      <div className="p-6">
        <p>Order not found.</p>
        <button onClick={() => navigate("/my-account/my-orders")} className="mt-3 px-4 py-2 bg-[#6F4D34] text-white rounded">Back</button>
      </div>
    );
  }

  const statusUpdates = order.statusUpdates || [
    { label: "Ordered", date: createdAt ? new Date(createdAt).toLocaleDateString() : "—" },
    { label: "Packed", date: "—" },
    { label: "Shipped", date: "—" },
    { label: "Out for Delivery", date: "—" },
    { label: "Delivered", date: order.DeliveredAt ? new Date(order.DeliveredAt).toLocaleDateString() : "—" },
  ];

  const originalPrice = order.MaxBudget || 0;
  const negotiated = order.ArtistNegotiatedBudgets?.[0] || order.finalPrice || order.totalAmount || finalPrice || originalPrice;
...
const deliveryAddress = order?.deliveryAddress || {};
        ...
        <div className="bg-white rounded-2xl border border-[#EBEBEB] p-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          <div className="flex-1 space-y-1">
            <h1 className="text-lg sm:text-2xl font-bold text-gray-800">
              {order.ProductName || unifiedItems[0]?.name || "Product"}
            </h1>
            <p className="text-sm text-gray-600">
              Order ID : <span className="text-gray-800">{orderId || "N/A"}</span>
            </p>
            <p className="text-sm text-gray-600">
              Order Date : <span className="text-gray-800">{createdAt ? new Date(createdAt).toLocaleString() : "N/A"}</span>
            </p>
            <p className="text-sm text-gray-600">
              Seller : <span className="text-gray-800">{sellerName || "Seller Name"}</span>
            </p>
          </div>

          <div className="text-left sm:text-right">
            <p className="text-lg sm:text-2xl font-semibold text-[#6F4D34]">₹{negotiated || 0}</p>
            <p className="text-sm text-gray-500 mt-1">Estimated Delivery</p>
        ...
        <div className="bg-white rounded-2xl border border-[#EBEBEB] p-4 sm:p-6 relative">
          ...
          {orderStatus === "Cancelled" ? (
            <div className="flex flex-col items-center justify-center gap-6">
              <div className="w-full sm:w-2/3 text-center py-8 px-4">
                <p className="text-2xl font-bold text-red-600">Order Cancelled</p>
                <p className="text-sm text-gray-600 mt-2">This order has been cancelled and is no longer in progress.</p>
              </div>
              <button className="bg-[#6F4D34] text-white px-5 py-2 rounded-lg hover:bg-[#5b3f2a] transition text-sm font-semibold">Chat with Us</button>
            </div>
          ) : (
            <>
              <div className="relative flex flex-col sm:flex-row items-center sm:px-2 gap-6 sm:gap-0">
                ...
                {statusUpdates.map((status, index) => {
                  ...
                  return (
                    <div key={index} className="relative z-10 flex flex-col sm:flex-1 sm:items-center sm:text-center min-w-[80px] w-full sm:w-auto">
                      <div className="hidden md:flex flex-col items-center text-center flex-1 min-w-[80px]">
                        <div className={`w-4 h-4 rounded-full border-2 mb-2 mt-0.5 ${isCurrent ? "bg-[#6F4D34] border-[#6F4D34]" : "bg-white border-gray-300"}`} />
                        <p className={`text-xs ${isCurrent ? "text-[#6F4D34] font-medium" : "text-gray-500"}`}>{status.label}</p>
                        <span className="text-[11px] text-gray-400">{status.date}</span>
                      </div>

                      {/* mobile */}
                      <div className="flex md:hidden items-start gap-3 relative w-full">
                        <div className={`w-3 h-3 rounded-full mt-1 flex-shrink-0 ${isCurrent ? "bg-[#6F4D34] border-2 border-[#6F4D34]" : "bg-gray-300"}`} />
                        <div className="flex flex-col text-left">
                          <p className={`text-[10px] leading-tight ${isCurrent ? "text-[#6F4D34] font-medium" : "text-gray-500"}`}>{status.label}</p>
                          <span className="text-[9px] text-gray-400">{status.date}</span>
                        </div>
                      </div>
                    </div>
                ...
                {/* <div className="absolute top-4 right-4 hidden sm:block">
                  <button className="text-xs sm:text-sm bg-[#6F4D34] text-white px-3 py-1.5 rounded-lg hover:bg-[#5b3f2a] transition whitespace-nowrap">Download Invoice</button>
                </div> */}
              </div>

              <div className="flex flex-wrap gap-5 mt-6">
                {orderStatus !== "Delivered" && (
                  <button onClick={() => setShowCancelModal(true)} className="bg-red-600 text-white px-4 py-2 rounded-lg hover:bg-red-700 transition text-sm font-semibold">Cancel Order</button>
                )}
                <button className="bg-[#6F4D34] text-white px-4 py-2 rounded-lg hover:bg-[#5b3f2a] transition text-sm font-semibold">Chat with Us</button>
                <button className="sm:hidden bg-[#6F4D34] text-white px-4 py-2 rounded-lg hover:bg-[#5b3f2a] transition text-sm font-semibold">Download Invoice</button>
              </div>
            </>
          )}
        </div>

        {/* ITEMS + DELIVERY ADDRESS + PRICE DETAILS */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* <div className="bg-white rounded-2xl p-4 sm:p-6 border border-[#EBEBEB]">
            <h4 className="text-md font-semibold text-gray-800 mb-4">Delivery Details</h4>
            ...
            {/* <div className="space-y-3 mb-4">
              {unifiedItems.map((it, idx) => (
                <div key={idx} className="flex items-center gap-3 border p-2 rounded">
                  <img src={buildImageUrl(it.mainImage) || "/images/placeholder.jpg"} className="w-16 h-16 object-cover rounded" alt={it.name} onClick={() => handleImageClick(it.mainImage)} />
                  <div className="flex-1">
                    <div className="text-sm font-semibold">{it.name || `Item ${idx + 1}`}</div>
            ...
            <div className="p-2 rounded-lg text-sm text-gray-700 space-y-1 leading-relaxed">
              <p className="font-semibold">{buyer?.name || buyer?.fullName || "N/A"} <br />
                <span className="text-gray-500 ">{buyer?.email || buyer?.username || ""}</span>
              </p>

              <p>{address?.line1 || address?.addressLine1 || "Address Line 1"}, {address?.line2 || ""}</p>
  ...
  {deliveryAddress && Object.keys(deliveryAddress).length > 0 ? (
    <div className="p-2 rounded-lg text-sm text-gray-700 space-y-1 leading-relaxed">
      ...
      {deliveryAddress.landmark ? <p>{deliveryAddress.landmark}</p> : null}
    ...
    <p className="text-gray-500 p-2">No address found for this order.</p>
  )}
</div>
          ...
          <div className="bg-white rounded-2xl p-4 sm:p-6 border border-[#EBEBEB]">
            ...
            <div className="space-y-2 text-sm text-gray-700">
              <div className="flex justify-between"><span>List Price</span><span className="line-through text-gray-400">₹{originalPrice}</span></div>
              <div className="flex justify-between"><span>Selling Price</span><span>₹{negotiated}</span></div>
              <div className="flex justify-between"><span>Special Discount</span><span className="text-green-600">-₹{discount}</span></div>
              <div className="flex justify-between"><span>Frame Included</span><span>{order.IsFramed ? "Yes" : "No"}</span></div>
              <div className="flex justify-between"><span>Payment Method</span><span>{order.PaymentTerm || order.paymentMethod || order.payment || "N/A"}</span></div>
              <hr className="my-2" />
              <div className="flex justify-between font-semibold text-[#6F4D34]"><span>Total Amount</span><span>₹{negotiated}</span></div>
            </div>
          </div>
        </div>
        ...
        <div id="review-section" className="bg-white rounded-2xl p-5 sm:p-6 flex flex-col gap-4 border border-[#EBEBEB]">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div className="flex gap-1 text-2xl">
              {[1, 2, 3, 4, 5].map((star) => (
                <span key={star} onClick={() => setRating(star)} className={`cursor-pointer transition-colors ${rating >= star ? "text-[#6F4D34]" : "text-gray-300"}`}>★</span>
              ))}
            </div>
          </div>
          ...
          <div className={`grid grid-cols-1 gap-4 sm:grid-cols-2 overflow-hidden transition-all duration-500 ease-in-out transform ${rating > 0 ? "max-h-[1000px] opacity-100 scale-100" : "max-h-0 opacity-0 scale-95"}`}>
            <div className="sm:col-span-2 flex flex-col sm:flex-row sm:items-center gap-4">
              <div className="flex-1">
                <div className="review-section grid grid-cols-1 sm:grid-cols-2 gap-4 w-full">
                  <div className="flex flex-col w-full">
                    <label className="text-sm font-medium mb-1">Title</label>
                    <input type="text" value={reviewTitle} onChange={(e) => setReviewTitle(e.target.value)} placeholder="Enter a title for your review" className="w-full px-3 py-2 border rounded-lg text-sm" />
                  </div>
                </div>
              </div>
              ...
              <div className="sm:col-span-2 mt-3 text-sm text-gray-700">
                <div className="flex gap-4 flex-wrap">
                  {uploadedFiles.map((file, index) => {
                    ...
                    return (
                      <div key={index} className="w-[200px] flex-shrink-0 relative bg-white rounded-xl pl-2 pr-2 pb-2 pt-4 border border-[#EBEBEB]">
                        <div className="absolute top-2 right-2">
                          <button type="button" title="Delete" onClick={() => handleDeleteImage(index)} className="px-2 py-1 rounded bg-gray-100 text-sm">Delete</button>
                        </div>

                        <div className="mb-2 text-xs text-gray-700 space-y-0.5 truncate">
                          <p title={file.name}><strong>Name:</strong> {(file.name && file.name.slice(0, 20)) || `Image ${index + 1}`}</p>
                        </div>

                        <img onClick={() => handleImageClick(imageUrl)} src={imageUrl} alt={`Preview ${index + 1}`} className="w-full h-[100px] object-cover rounded-md border" />
                      </div>
                    );
                  })}
                </div>
            ...
            <div className="sm:col-span-2">
              <label className="text-sm font-medium text-gray-700 block mb-1 ml-1">Description</label>
              <textarea rows="4" value={reviewDescription} onChange={(e) => setReviewDescription(e.target.value)} placeholder="Write your experience in detail..." className="w-[95%] ml-1 border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#6F4D34]"></textarea>
            </div>

            {!hasSubmittedReview && (
              <div className="sm:col-span-2 text-right">
                <button onClick={handleSubmitReview} className="bg-[#6F4D34] text-white px-5 py-2 rounded-lg hover:bg-[#5b3f2a] transition text-sm font-semibold">Submit Review</button>
              </div>
            )}
          </div>
        </div>
        ...
        <AnimatePresence>
          <motion.div className="fixed inset-0 flex items-center justify-center bg-black/40 backdrop-blur-sm z-50" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} transition={{ duration: 0.15 }} className="bg-white rounded-xl shadow-lg p-6 w-full max-w-md">
              ...
              <label className="block text-sm font-medium text-gray-700 mb-1">Comment</label>
              <textarea rows="3" value={cancelComment} onChange={(e) => setCancelComment(e.target.value)} placeholder="Add any details (optional)" className="w-full border rounded-lg px-3 py-2 text-sm mb-4"></textarea>

              <div className="flex justify-end gap-3">
                <button onClick={() => setShowCancelModal(false)} className="px-4 py-2 rounded-lg border text-sm">Close</button>
                <button onClick={handleCancelOrder} disabled={loadingCancel} className="bg-red-600 text-white px-4 py-2 rounded-lg hover:bg-red-700 text-sm font-semibold disabled:opacity-50">
                  {loadingCancel ? "Cancelling..." : "Confirm Cancel"}
                </button>
              </div>
            </motion.div>
      ...
      {/* IMAGE POPUP */}
      {showPopup && (
        <div onClick={() => setShowPopup(false)} style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "rgba(0,0,0,0.85)", display: "flex", justifyContent: "center", alignItems: "center", zIndex: 1000 }}>
          <div onClick={(e) => e.stopPropagation()} style={{ position: "relative", height: "70%", backgroundColor: "#111", borderRadius: "12px", boxShadow: "0 0 20px rgba(255,255,255,0.2)", display: "flex", justifyContent: "center", alignItems: "center", overflow: "hidden", width: "80%" }}>
            <img src={currentImages[currentImageIndex]} alt="Popup" style={{ width: "100%", height: "100%", objectFit: "cover", borderRadius: "12px" }} />
          </div>
        </div>
      )}
    </div>
