...
const AddCustomRequestForm = () => {
  ...
  const handleNegotiationAction = async (actionType) => {
    ...
    const payload = {
      NegotiatedBudget,
      Notes,
      BuyerStatus: actionType === 'save' ? selectedRequest.BuyerStatus : actionType,
    };

    try {
      // Send update to correct negotiation endpoint
      const updatedData = await putAPI(`/api/update-negiotaite-Buyer-budget/${selectedRequest._id}`, payload);
            ...
            <form className="space-y-6">
              ...
              <div className="flex justify-between pt-4 border-t mt-4 text-lg px-3 font-semibold">
                ...
                <button
                  ...
                  onClick={() => handleNegotiationAction('Approved')}
                >
                  Accept
                </button>
                <button
