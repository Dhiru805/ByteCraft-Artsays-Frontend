...
const Partner = () => {
  ...
  const fetchSEOMetadata = async () => {
    try {
      const response = await axiosInstance.get("/api/partner/getSEO");
      const meta = response.data?.data || {};

      setSeoData({
        metaTitle: meta.metaTitle || "Partner with Artsays",
        metaDescription:
          meta.metaDescription ||
          "Join Artsays as a brand partner, gallery, or museum. Let's create meaningful experiences and reach a global audience together.",
        metaKeywords:
      ...
      console.error("Error fetching Partner SEO metadata:", error);
      // Fallback SEO in case API fails
      setSeoData({
        metaTitle: "Partner with Artsays",
        metaDescription: "Join Artsays as a brand partner. Let's create meaningful experiences together.",
        metaKeywords: "partnership, brand partner, artsays",
        metaAuthor: "Artsays",
    ...
    <div className="w-full font-[poppins]">
      <Helmet>
        ...
        <script type="application/ld+json">
          {JSON.stringify({ "@context": "https://schema.org", "@type": "BreadcrumbList", "itemListElement": [{ "@type": "ListItem", "position": 1, "name": "Home", "item": "https://artsays.in/" }, { "@type": "ListItem", "position": 2, "name": "Partner", "item": "https://artsays.in/partner" }] })}
        </script>
      </Helmet>
      <nav aria-label="breadcrumb" className="max-w-[1440px] mx-auto px-4 md:px-8 flex items-center gap-2 text-sm text-gray-500 py-3">
        ...
        <span className="text-[#6F4D34] font-semibold" aria-current="page">Partner</span>
      </nav>
      <HeroImgPartner />
      <PartnerContent />
