...
const ProductDetails = () => {
  ...
  const navigateToArtistProfile = (artist) => {
    if (!artist) return;
    const profileSlug =
      artist.username || `${artist.name}_${artist.lastName}_${artist._id}`;
    navigate(`/artsays-community/profile/${profileSlug}`, {
      state: { userId: artist._id },
    });
  };
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        ...
        const products1 =
          res1?.data?.data?.filter((p) => p && p.status === "Approved") || [];
        const products2 =
          res2?.data?.data?.filter((p) => p && p.status === "Approved") || [];
        ...
        allProducts = allProducts
          .map((p) => {
            ...
            const matchedRating = ratings.find(
              (r) => r && String(r.productId) === String(p._id),
            );
            return {
              ...p,
        ...
        allProducts = allProducts.map((p) => {
          const match = badgeData.find(
            (b) => b && String(b._id) === String(p._id),
          );
          return {
            ...p,
        ...
        if (!found) {
          try {
            ...
            if (directProduct && directProduct._id) {
              const matchedRating = ratings.find(
                (r) => r && String(r.productId) === String(directProduct._id),
              );
              const matchedBadge = badgeData.find(
                (b) => b && String(b._id) === String(directProduct._id),
              );
              found = {
                ...directProduct,
                averageRating: matchedRating?.averageRating
        ...
        const gallery = [
          found.mainImage
            ? resolveMediaUrl(found.mainImage)
  ...
  useEffect(() => {
    const fetchUserAddresses = async () => {
      ...
      try {
        const res = await getAPI(`/api/get-address/${userId}`, {}, true, false);
    ...
    const fetchDefaultOrigin = async () => {
      ...
      try {
        const res = await getAPI(
          `/api/get-default-shipping-address-TAT/${productId}`,
          {},
          true,
          false,
        );
        ...
        if (defaultData?.addressId?.pincode) {
          const pin = defaultData.addressId.pincode;
          setOriginPin(pin);
          setDefaultShippingAddress(defaultData);
          console.log("✅ Origin PIN (Default Shipping):", pin);
        } else {
          // Fallback if no default address set for this product
          const fallbackPin =
            product?.pincode || product?.seller?.pincode || "110017";
          setOriginPin(fallbackPin);
          console.log("⚠️ Using fallback origin PIN:", fallbackPin);
        }
      } catch (err) {
        console.error("Failed to fetch default shipping address:", err);
        setOriginPin(product?.pincode || "110017");
      }
    };

    if (product) fetchDefaultOrigin();
  }, [productId, product]);

  const fetchTAT = async (destinationPin) => {
    ...
    try {
      ...
      console.log("tat data",res)

      if (res?.data && !res.data.hasError) {
        setTatData(res.data.data.data|| res.data);
      } else {
        toast.warning(res?.data?.message || "Could not calculate TAT");
      }
    } catch (err) {
      console.error("TAT Error:", err);
      toast.error("Failed to get expected delivery time");
    } finally {
      setTatLoading(false);
    }
  };
...
useEffect(() => {
  if (selectedAddress?.pincode) {
    fetchTAT(selectedAddress.pincode);
  }
}, [selectedAddress]);  
...
const handleAddressSelect = (addr) => {
  ...
  setManualPin("");                    
  const display = `${addr.addressLine1 || ""}, ${addr.city}, ${addr.state} - ${addr.pincode}`;
  setAddressDisplay(display);

};


const handleManualPinSubmit = async (e) => {
  e.preventDefault();
  if (manualPin.length !== 6) {
  ...
  setAddressDisplay(`Manual PIN: ${manualPin}`);

};

  const mainCategoryName = useMemo(
    () =>
      categoryData.mainCategories.find(
        (c) => c && String(c._id) === String(product?.mainCategory),
  ...
  const categoryName = useMemo(
    () =>
      categoryData.categories.find(
        (c) => c && String(c._id) === String(product?.category),
      )?.categoryName || "N/A",
    [product, categoryData],
  );
  ...
  const subCategoryName = useMemo(
    () =>
      categoryData.subCategories.find(
        (c) => c && String(c._id) === String(product?.subCategory),
      )?.subCategoryName || "N/A",
    [product, categoryData],
  );
    ...
    return reviews
      .filter((review) => {
        ...
        if (buyerRequest && buyerRequest.ProductName) {
          if (
            buyerRequest.ProductName.trim().toLowerCase() === currentProductName
          )
            return true;
        }
        if (
          review.productNameSnapshot &&
          review.productNameSnapshot.trim().toLowerCase() === currentProductName
        )
          return true;
        return false;
      })
  ...
  return (
    <div className="bg-[#F9F7F5] min-h-screen font-[poppins] text-[#1A1A1A]">
      <Helmet>
        ...
        <meta
          property="og:title"
          content={`${product.productName} | Artsays`}
        />
        <meta
      ...
      <div className="max-w-[1440px] mx-auto px-4 md:px-8 py-6">
        ...
        <nav className="flex items-center gap-2 text-sm text-gray-500 mb-8 overflow-x-auto whitespace-nowrap scrollbar-hide">
          <button
            onClick={() => navigate("/")}
            className="hover:text-[#6F4D34] transition-colors"
          >
            Home
          </button>
          <ChevronRight size={14} />
          <span className="hover:text-[#6F4D34] cursor-pointer">
            {mainCategoryName}
          </span>
          <ChevronRight size={14} />
          <span className="text-[#6F4D34] font-semibold">
            {product.productName}
          </span>
        </nav>
          ...
          <div className="lg:col-span-5">
            <div className="sticky top-6 space-y-6">
              ...
              <PurchaseCard
                ...
                handleManualPinSubmit={handleManualPinSubmit}
                tatData={tatData}
                tatLoading={tatLoading}
                onAddToCart={() => {
                  if (ensureBuyer()) addToCart(product._id, quantity);
                }}
                onBuyNow={() => {
                  if (ensureBuyer()) {
                    const giftWrapAmount =
                      product.giftWrapping && giftOption
                        ? product.giftWrappingCostAmount || 0
                        : 0;
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-8 border-t border-gray-50">
                    {product.inspirationSource && (
                      <div className="space-y-3">
                        <h4 className="text-sm font-black text-[#6F4D34] uppercase tracking-widest">
                          The Inspiration
                        </h4>
              ...
              <div className="flex flex-col md:flex-row gap-8 items-start">
                  ...
                  <img
                    src={
                      product.userId?.profilePhoto
                        ? resolveMediaUrl(product.userId.profilePhoto)
                ...
                <div className="space-y-4 flex-grow">
                  ...
                  <p className="text-lg text-gray-600 leading-relaxed italic">
                    {product.artistBio ||
                      "A passionate artist sharing unique visions through Artsays."}
                  </p>
                  <div className="flex flex-wrap gap-4 pt-4">
                    ...
                    {(!userId || userType === "Buyer") && (
                      <button
                        onClick={() =>
                          navigate(
                            `/my-account/custom-request?artistId=${product.userId?._id}`,
                          )
                        }
      ...
      >
        ...
        <div>
          ...
          <div className="flex items-center gap-1">
            <span className="font-bold text-gray-900 group-hover:text-[#6F4D34] transition-colors">
              {username}
            </span>
            {product.userId?.verified && (
        ...
        <ChevronRight
          size={18}
          className="ml-auto text-gray-300 group-hover:text-[#6F4D34] transition-all"
        />
      </div>
        ...
        <div className="fixed inset-0 z-[1000] bg-black/90 backdrop-blur-xl flex items-center justify-center p-4">
          <div className="relative w-full max-w-6xl aspect-video bg-[#1a1a1a] rounded-[40px] overflow-hidden shadow-2xl">
            <button
              ...
              className="absolute top-8 right-8 z-20 w-12 h-12 bg-white/10 text-white rounded-full hover:bg-white/20 transition-all flex items-center justify-center"
            >
              ✕
            </button>
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="relative group">
                ...
                <div className="absolute -bottom-10 left-1/2 -translate-x-1/2 bg-white/10 backdrop-blur-md px-4 py-2 rounded-xl text-white text-xs font-medium">
                  {product.dimensions?.width || 100} x{" "}
                  {product.dimensions?.height || 70} cm
                </div>
              </div>
            </div>
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2">
        {product.condition === "Resale" && (
          <span className="px-3 py-1 bg-orange-100 text-orange-700 border border-orange-300 rounded-full text-xs font-bold uppercase tracking-wider">
            Resale
          </span>
        )}
        {product.tags?.map((t, i) => (
          <span
            key={i}
            className="px-3 py-1 bg-white border border-gray-100 rounded-full text-xs font-semibold text-gray-500"
          >
            #{t}
          </span>
        ))}
        {product.editionType && (
          <span className="px-3 py-1 bg-[#6F4D34]/5 text-[#6F4D34] rounded-full text-xs font-bold uppercase tracking-wider">
            {product.editionType}
          </span>
        )}
      </div>
        ...
        <div className="relative space-y-6">
          <div className="flex items-center justify-between">
            <div className="space-y-1">
              <p className="text-[10px] font-black text-[#6F4D34] uppercase tracking-[0.2em]">
          ...
          {discountPercent > 0 && (
            <div className="flex items-center gap-2 text-sm">
              ...
              <span className="text-gray-400 line-through font-bold decoration-[#6F4D34]/30">
    ...
    <div className="bg-white rounded-[32px] border border-gray-200 shadow-xl overflow-hidden">
      <div className="p-6 space-y-6">
        ...
        <div className="space-y-4">
          {product.quantity > 1 && (
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center border border-gray-200 rounded-xl overflow-hidden">
                ...
                <span className="px-6 py-2 text-sm font-bold bg-gray-50 min-w-[3rem] text-center">
                  {quantity}
                </span>
                <button
                  onClick={() =>
                    setQuantity(Math.min(product.quantity || 10, quantity + 1))
                  }
                  className="px-4 py-2 hover:bg-gray-50 text-gray-600 font-bold"
                >
                  +
          ...
          {/* Protection & Gift Options */}
          <div className="space-y-3">
            {product.insuranceCoverage && (
              <div
                className="flex items-center gap-3 p-3 rounded-2xl bg-gray-50 border border-gray-100 cursor-pointer hover:border-[#6F4D34]/20 transition-all"
                onClick={() => setProtection(!protection)}
              >
                <div
                  className={`w-5 h-5 rounded-md border-2 flex items-center justify-center transition-all ${protection ? "bg-[#6F4D34] border-[#6F4D34]" : "border-gray-300 bg-white"}`}
                >
                  {protection && (
                ...
                <div className="flex-grow">
                  <p className="text-xs font-bold text-gray-800 leading-tight">
                    Delivery Protection Plan
                  </p>
                  <p className="text-[10px] text-gray-400">
                    Insures against transit damage for ₹2,749
                  </p>
                </div>
                <ShieldCheck size={18} className="text-[#6F4D34]" />
              </div>
            ...
            {product.giftWrapping && (
              <div
                className="flex items-center gap-3 p-3 rounded-2xl bg-gray-50 border border-gray-100 cursor-pointer hover:border-[#6F4D34]/20 transition-all"
                onClick={() => setGiftOption(!giftOption)}
              >
                <div
                  className={`w-5 h-5 rounded-md border-2 flex items-center justify-center transition-all ${giftOption ? "bg-[#6F4D34] border-[#6F4D34]" : "border-gray-300 bg-white"}`}
                >
                  {giftOption && (
                    <span className="text-white text-[10px]">✓</span>
                  )}
                ...
                <div className="flex-grow">
                  ...
                  <p className="text-[10px] text-gray-400">
                    Luxury packaging & custom note for ₹
                    {product.giftWrappingCostAmount?.toLocaleString() ||
                      "2,749"}
                  </p>
                </div>
                <Gift size={18} className="text-[#6F4D34]" />
        ...
        <div className="space-y-5 pt-2">
          ...
          {/* Manual PIN */}
          <form onSubmit={handleManualPinSubmit} className="relative group">
            <MapPin
              size={16}
            ...
            <input
              ...
              className="w-full h-12 pl-11 pr-12 bg-gray-50 border border-gray-100 rounded-2xl text-sm font-medium focus:outline-none focus:ring-2 focus:ring-[#6F4D34]/10 focus:border-[#6F4D34]"
            />
            <button
              type="submit"
              className="absolute right-2 top-1/2 -translate-y-1/2 w-8 h-8 flex items-center justify-center text-[#6F4D34] hover:bg-[#6F4D34] hover:text-white rounded-lg transition-all"
            >
              <ArrowRight size={18} />
            </button>
          </form>
          ...
          {addressDisplay && (
            <div className="flex items-start gap-3 p-4 bg-[#6F4D34]/5 rounded-2xl border border-[#6F4D34]/10">
              <MapPin size={18} className="text-[#6F4D34] shrink-0 mt-0.5" />
              <div className="flex-grow">
                <p className="text-xs font-bold text-[#6F4D34]">
                  Delivering to
                </p>
              ...
              <button
                ...
                className="text-xs font-black text-[#6F4D34] uppercase underline self-start"
              >
                Change
              </button>
            </div>
          ...
          {/* TAT Display */}
          {tatLoading && (
            <div className="text-[#6F4D34] text-sm py-2">
          ...
          {/* {tatData && !tatLoading && (
            <div className="flex items-center gap-3 p-4 bg-green-50 border border-green-200 rounded-2xl">
              ...
              <div>
                <p className="font-bold text-green-800">Expected Delivery</p>
          ...
          <div className="flex items-center gap-3 text-[10px] text-gray-400 font-bold uppercase tracking-wider px-2">
            ...
            <div className="flex items-center gap-1">
              <Clock size={12} /> 
              {/* {product.estimatedDelivery || "2-5"}  */}
          {tatData.tat
  ? `${tatData.tat}-${Number(tatData.tat) + 3} Days`
  : ""}
        ...
        <div className="grid grid-cols-2 gap-4">
          <button
            onClick={onAddToCart}
            disabled={!product.quantity}
            className="h-14 rounded-2xl border-2 border-[#6F4D34] text-[#6F4D34] font-black text-sm uppercase tracking-widest hover:bg-[#6F4D34] hover:text-white transition-all flex items-center justify-center gap-2 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <ShoppingCart size={18} /> Cart
          </button>
          <button
      ...
      {!product.quantity && (
        <div className="bg-red-500 text-white text-center py-2 text-[10px] font-black uppercase tracking-[0.2em]">
          Currently Sold Out
        </div>
      )}
    </div>
          ...
          {offersData.map((offer, i) => (
            <div key={i} className="min-w-full px-1">
              <div className="bg-white p-4 rounded-2xl border-2 border-dashed border-gray-100 hover:border-[#6F4D34]/20 transition-all">
                ...
                <span className="text-[10px] font-black text-[#6F4D34] uppercase px-2 py-0.5 bg-[#6F4D34]/5 rounded-md">
                  {offer.offers}
                </span>
              </div>
            </div>
...
const DetailsGrid = ({
      ...
      return (
        <div className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl border border-gray-100/50">
          ...
          <a
            ...
            className="text-sm font-bold text-[#6F4D34] hover:underline flex items-center gap-1"
          >
  ...
  const isNFT =
    categoryInfo.categoryName === "NFT Art" ||
    categoryInfo.subCategoryName === "NFT Art";
    ...
    <div className="space-y-12">
      <Section title="Artwork Identity" icon={Box}>
        ...
        {product.productType === "Limited Edition" && (
          <Item label="Edition Number" value={product.editionNumber} />
        )}
      ...
      <Section title="Technical Details" icon={Award}>
        ...
        <Item
          ...
          value={product.customEngravingAvailable}
        />
      </Section>
      ...
      <Section title="Trust & Compliance" icon={ShieldCheck}>
        ...
        <Item
          ...
          value={product.ownershipConfirmation}
        />
        ...
        {product.includeGst && (
          <Item label="GST Percentage" value={`${product.gstPercentage}%`} />
        )}
      ...
      {isNFT && (
        <Section title="NFT Specifications" icon={Zap}>
          ...
          <Item
            label="Royalty"
            value={
              product.royaltyPercentage ? `${product.royaltyPercentage}%` : null
            }
          />
          ...
          {product.unlockableContent && (
            <Item
              label="Unlockable Link"
              value={product.unlockableContentLink}
              isLink
            />
          )}
      ...
      <Section title="Shipping & Returns" icon={Truck}>
        ...
        <Item
          ...
          value={
            product.shippingCharges ? `₹${product.shippingCharges}` : "Free"
          }
        />
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      {reviews.map((review, i) => (
        <div
          key={i}
          className="p-6 bg-white rounded-[32px] border border-gray-100 shadow-sm hover:shadow-md transition-all"
        >
                ...
                <div className="flex items-center gap-1">
                  {[...Array(5)].map((_, j) => (
                    <Star
                      key={j}
                      size={12}
                      fill={j < review.rating ? "#facc15" : "none"}
                      className={
