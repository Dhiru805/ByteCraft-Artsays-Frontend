...
const ProductDetails = () => {
  ...
  const artworkSize = { width: 100, height: 70 };
  const roomBackgrounds = ["/artimages/viewintheroom.jpg", "/artimages/wall3.jpg", "/artimages/wall4.webp"];
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        ...
        const ratings = ratingRes?.data?.data || [];
        allProducts = allProducts.map((product) => {
          const matchedRating = ratings.find((r) => String(r.productId) === String(product._id));
          return {
            ...product,
        ...
        const gallery = [
          found.mainImage ? `${imageBaseURL}${found.mainImage}` : "/herosectionimg/placeholder.png",
          ...(found.otherImages || []).map((img) => `${imageBaseURL}${img}`),
        ];
  ...
  const nextSlide = () => { if (index < offersData.length - 3) setIndex(index + 1); };
  const prevSlide = () => { if (index > 0) setIndex(index - 1); };

  const handleShare = () => {
    if (navigator.share) {
      navigator.share({ title: "Check out this artwork!", text: "I found this amazing art on Artsays!", url: window.location.href }).catch(() => {});
    } else {
      alert("Share not supported in this browser");
    }
  };

  if (loading) return <div className="max-w-[1440px] mx-auto p-10 text-center text-xl font-semibold">Loading product…</div>;
  if (!product) return <div className="max-w-[1440px] mx-auto p-10 text-center text-xl font-semibold">Product not found.</div>;

  const discountPercent = calculateDiscount(product.sellingPrice, product.marketPrice);
  const username = `${product?.userId?.username || `${product?.userId?.name || ""} ${product?.userId?.lastName || ""}`}`.trim();
  const artistName = `${product?.userId?.name || ""} ${product?.userId?.lastName || ""}`.trim();

  const ProductImages = ({ imagesProp, initialImage }) => {
    const [selectedImage, setSelectedImage] = useState(initialImage || imagesProp[0] || "/herosectionimg/placeholder.png");
    const [selectedRoom, setSelectedRoom] = useState(roomBackgrounds[0]);
    const [showPopup, setShowPopup] = useState(false);
    ...
    const changeImage = (direction) => {
      ...
      if (direction === "next") setSelectedImage(imagesProp[(currentIndex + 1) % imagesProp.length]);
      else setSelectedImage(imagesProp[(currentIndex - 1 + imagesProp.length) % imagesProp.length]);
    };

    return (
      <div className="max-w-[1440px] mx-auto font-[Poppins] bg-[#ffffff] text-[#111] p-6">
        <p className="text-sm text-gray-500">Art / {product.mainCategory || "Painting"} / {product.category || ""}</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-10 gap-6 mt-3">
          <div className="col-span-8">
            <div className="grid grid-cols-1 md:grid-cols-10 gap-6 mt-3">
              ...
              <div className="flex flex-col lg:flex-row-reverse col-span-5 gap-3 relative">
                <div className="relative w-full h-auto align-content-center product-card">
                  <img src={selectedImage} alt="Main" className="w-full h-[550px] object-contain product-img transition-all duration-300" />

                  <button onClick={() => setShowPopup(true)} className="absolute bottom-5 bg-[#48372D] text-white text-sm px-3 py-1 rounded-2xl shadow flex justify-self-center gap-1">👁️ View in Room</button>

                  <button onClick={handleShare} className="absolute top-3 right-3 text-[#48372D] text-4xl py-1"><BsTelegram /></button>
                ...
                <div className="flex lg:flex-col gap-3 overflow-x-auto lg:overflow-y-auto lg:overflow-x-hidden lg:max-h-[550px] scrollbar-hide">
                  {imagesProp.map((img, i) => (
                    <img key={i} src={img} alt={`thumb-${i}`} onClick={() => setSelectedImage(img)} className={`w-24 h-24 object-contain rounded-lg product-img product-card cursor-pointer border-2 transition-all duration-200 ${selectedImage === img ? "border-[#48372D]" : "border-transparent"}`} />
                  ))}
                </div>
              </div>
                    ...
                    <div className="relative w-full h-[550px] overflow-hidden rounded-lg">
                      ...
                      <div className="absolute bottom-[200px] inset-0 flex justify-center items-center">
                        <div className="relative">
                          ...
                          <div className="absolute -bottom-6 w-full text-center text-sm font-medium bg-white py-1 rounded-md">{artworkSize.width} × {artworkSize.height} cm</div>
                        </div>
                      </div>
                    </div>
              ...
              {/* ===== Right: Product Info ===== */}
              <div className="col-span-5">
                <h1 className="text-lg md:text-3xl font-semibold leading-snug">{product.productName}</h1>

                <div className="flex flex-wrap gap-2 text-xs mt-2">
                  {product.tags?.length ? product.tags.map((t, i) => <span key={i} className="bg-gray-200 px-2 py-1 rounded-md">#{t}</span>) : <span className="bg-gray-200 px-2 py-1 rounded-md">#Art</span>}
                </div>

                <div className="flex flex-wrap gap-2 text-xs mt-2">
                  <span className="bg-gray-200 px-2 py-1 rounded-md">{product.editionType || "Original Artwork"}</span>
                </div>

                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-dark font-bold text-lg flex mr-3 mt-2 items-center">{username} {product?.userId?.verified && <MdVerified className="ml-1 text-blue-600 w-4 h-4" />} {product.badges?.map((img, index) => (
                          <img
                            key={index}
                            src={`${imageBaseURL}${img}`}
                            className="w-4 h-4 rounded-full object-contain"
                    ...
                    <div className="flex items-center text-yellow-500 mt-2">
                      ...
                      <span className="text-dark ml-1 text-sm">({product.reviewCount ?? 0})</span>
                      <span className="text-dark text-sm ml-2"><strong>600+</strong> bought this month</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3 mt-2">
                  {discountPercent > 0 && <span className="bg-[#DC3545] px-2 py-1 rounded-md text-white font-bold text-md">-{discountPercent}%</span>}
                  <p className="text-3xl font-bold">₹{product.sellingPrice?.toLocaleString()}</p>
                  {product.marketPrice && <p className="text-dark line-through text-lg">₹{product.marketPrice?.toLocaleString()}</p>}
                </div>

                <div className="flex items-center gap-3 mt-2">
                  <div className="text-gray-500 text-xs">M.R.P.: <span className="line-through">₹{product.marketPrice?.toLocaleString() || "—"}</span></div>
                  <div className="text-gray-500 text-xs">Inclusive of all taxes</div>
                </div>

                <div className="flex gap-2 pt-2 justify-between">
                  <div className="p-2">
                    <img src="/herosectionimg/limited edition.png" alt="limited" className="w-full h-10 object-contain" />
                    <p className="text-dark text-center text-xs mt-2">Limited Edition</p>
                  </div>
                  <div className="p-2">
                    <img src="/herosectionimg/free delivery.png" alt="free" className="w-full h-10 object-contain" />
                    <p className="text-dark text-center text-xs mt-2">Free Delivery</p>
                  </div>
                  <div className="p-2">
                ...
                <div className="w-full max-w-sm d-block d-md-none mt-3">
                  <div className="flex items-start gap-2 mt-2 text-sm text-dark">
                    <MapPin size={18} className="text-dark mt-1" />
                    <p>{product.addressLine1 || "23 Aurum Lane, Sector 17, Vasant Vibe, New Delhi"}</p>
                  </div>
                  ...
                  <div className="mt-3">
                    ...
                    <table className="mt-2 w-full text-sm">
                      <tbody>
                        ...
                        <tr className="border-b border-gray-100">
                          <td className="text-xs font-semibold py-1">Sold by</td>
                          <td className="text-left text-xs text-orange-600 font-medium py-1 flex items-center gap-1">{username} {product?.userId?.verified && <MdVerified className="text-blue-600 w-4 h-4" />} </td>
                        </tr>
                        <tr>
                          <td className="text-xs font-semibold py-1">Artist</td>
                          <td className="text-left text-xs text-orange-600 font-medium py-1">{artistName || "N/A"}</td>
                        </tr>
                      </tbody>
                    </table>
                  </div>
                ...
                <div className="w-full mx-auto mt-6">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2"><HiMiniPercentBadge className="text-red-500 text-xl" /><h2 className="text-lg font-semibold">Offers</h2></div>
                    <div className="flex gap-2">
                      <button onClick={prevSlide} className={`p-2 rounded-full focus:bg-none ${index === 0 ? "opacity-40 cursor-not-allowed" : ""}`}><ChevronLeft /></button>
                      <button onClick={nextSlide} className={`p-2 rounded-full focus:bg-none ${index >= offersData.length - 3 ? "opacity-40 cursor-not-allowed" : ""}`}><ChevronRight /></button>
                    </div>
                  </div>
            ...
            <div className="mt-12 border-b">
              <div className="flex gap-8 text-[#48372D] font-medium text-lg border-b border-gray-200 overflow-x-auto no-scrollbar">
                {["description", "details", "artist", "reviews"].map((tab) => (
                  <button key={tab} onClick={() => setActiveTab(tab)} className={`pb-2 flex-shrink-0 transition-all duration-200 whitespace-nowrap ${activeTab === tab ? "border-b-4 border-[#48372D] font-semibold text-[#48372D]" : "font-semibold text-[#48372D]"}`}>
                    {tab.charAt(0).toUpperCase() + tab.slice(1)}
                  </button>
                ))}
              </div>
              ...
              <div className="py-6 text-gray-700 leading-relaxed text-sm">
                ...
                {activeTab === "details" && (
                  <div className="space-y-2">
                    <h2 className="text-xl font-semibold">Artwork Specifications</h2>
                    <p><span className="font-semibold">Material:</span> {product.medium || "N/A"}</p>
                    <p><span className="font-semibold">Dimensions:</span> {product.dimensions?.width ? `${product.dimensions.width} x ${product.dimensions.height} cm` : product.dimensions || "N/A"}</p>
                ...
                {activeTab === "artist" && (
                  <div>
                    <p className="font-semibold text-lg">Artist: {artistName || "N/A"}</p>
                    <p className="mt-2">{product.artistBio || "Artist details not available."}</p>
                  </div>
                )}
                ...
                {activeTab === "reviews" && (
                  <div className="space-y-4">
                    <div className="border p-3 rounded-lg shadow-sm">
                      <p className="font-semibold">Aarav Patel ⭐⭐⭐⭐⭐</p>
                      <p className="text-sm mt-1">Beautifully crafted! The attention to detail is stunning and it brings a calming energy to my home.</p>
                    </div>
                    <div className="border p-3 rounded-lg shadow-sm">
                      <p className="font-semibold">Priya Mehta ⭐⭐⭐⭐☆</p>
                      <p className="text-sm mt-1">Loved it! Just wished it came in a slightly bigger size.</p>
                    </div>
                  </div>
                )}
              </div>
          ...
          <div className="col-span-2 d-none d-md-block">
            <div className="sticky top-10">
              <div className="w-full max-w-sm bg-white rounded-2xl shadow-lg border border-dark px-3 py-3">
                <div className="flex justify-between items-center">
                  <div><p className="text-3xl font-bold">₹ {product.sellingPrice?.toLocaleString() || "—"}</p></div>
                  <button><Heart className="text-dark text-lg" /></button>
                </div>

                <p className="text-sm mt-1">FREE delivery <span className="font-semibold">{product.estimatedDelivery || "2–5 days"}</span>. <span className="text-orange-600 cursor-pointer">Details</span></p>

                <div className="max-w-md mt-3">
                  {!address ? (
                    <form onSubmit={handleSubmit} className="relative w-full">
                      <MapPin className="absolute left-0 top-1/2 -translate-y-1/2 text-gray-400 text-lg" />
                      <input type="text" placeholder="Enter Pin Code" value={pinCode} onChange={(e) => setPinCode(e.target.value)} className="w-full !border-b-2 py-2 pl-6 pr-10 text-gray-400 placeholder-dark-400 focus:outline-none" />
                      <button type="submit" className="absolute right-1 top-1/2 -translate-y-1/2 text-gray-400"><ArrowRight size={20} /></button>
                    </form>
                  ) : (
                    <div className="flex items-start gap-2 mt-2 text-sm text-dark">
                      <MapPin className="text-dark text-xl" />
                ...
                <div className="mt-2">
                  ...
                  <table className="mt-2 w-full text-sm">
                    <tbody>
                      ...
                      <tr className="border-b border-gray-100">
                        <td className="text-xs font-semibold py-1">Sold by</td>
                        <td className="text-left text-xs text-orange-600 font-medium py-1 flex items-center gap-1">{username} {product?.userId?.verified && <MdVerified className="text-blue-600 w-4 h-4" />}{product.badges?.map((img, index) => (
                          <img
                            key={index}
                            src={`${imageBaseURL}${img}`}
                            className="w-4 h-4 rounded-full object-contain"
                ...
                <div className="mt-3">
                  ...
                  <div className="mt-2 space-y-2">
                    <div className="flex items-start gap-2 text-sm cursor-pointer">
                      <input type="checkbox" checked={protection} onChange={() => setProtection(!protection)} className="mt-1 accent-black" />
                      <span>Screen Damage Protection while delivery for <span className="font-semibol">₹2,749.00</span></span>
                    </div>
                    <div className="flex items-start gap-2 text-sm cursor-pointer">
                      <input type="checkbox" checked={giftOption} onChange={() => setGiftOption(!giftOption)} className="mt-1 accent-black" />
                      <span>Gift options for <span className="font-semibold">₹2,749.00</span></span>
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-3 flex items-center justify-around">
                  <span className="font-semibold text-sm">Quantity :</span>
                  <select value={quantity} onChange={(e) => setQuantity(Number(e.target.value))} className="border border-dark rounded-md px-3 py-1 text-sm focus:outline-none">
                    {[...Array(10)].map((_, i) => <option key={i + 1} value={i + 1}>{i + 1}</option>)}
                  </select>
                </div>

                <div className="mt-3 flex flex-col gap-3">
                  <button className="flex items-center justify-center gap-2 flex-1 border border-dark rounded-full text-dark py-2 font-semibold add-cart"><ShoppingCart size={18} /> Add to Cart</button>
                  <button className="flex items-center justify-center gap-2 flex-1 hover:border-dark rounded-full bg-red-500 text-white py-2 font-semibold buy-now"><Zap size={18} /> Buy Now</button>
                </div>
              </div>
            </div>
          </div>
