...
const UpdateAffiliate = () => {
  ...
  useEffect(() => {
    if (!page) {
      toast.error("No Affiliate page data provided.");
      navigate("/super-admin/affiliate");
      return;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.data) {
        toast.success("Affiliate page updated successfully!");
        navigate("/super-admin/affiliate", { state: { reload: true } });
      } else {
        toast.error(response.data.message || "Failed to update Affiliate page");
      }
    } catch (err) {
              ...
              <form onSubmit={handleSubmit} encType="multipart/form-data">
                ...
                <div className="d-flex align-items-center mb-3" style={{ gap: "20px" }}>
                  ...
                  <button type="submit" className="btn btn-primary" disabled={loading}>
                    {loading ? "Updating..." : "Update Affiliate Page"}
                  </button>
                </div>
              </form>
            </div>
