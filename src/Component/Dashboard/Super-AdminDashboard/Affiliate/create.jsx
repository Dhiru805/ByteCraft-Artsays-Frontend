...
function AffiliateCreate() {
  ...
  const [formData, setFormData] = useState({
    ...
    cardsHeading: "",
    cardsDescription: "",
    articles: [{ articleHeading: "", articleContent: "", bannerImage: null }],
    cards: [],
  });
  ...
  const addArticle = () => {
    setFormData({
      ...formData,
      articles: [...formData.articles, { articleHeading: "", articleContent: "", bannerImage: null }],
    });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (hasCardsSection) {
        ...
        if (!formData.cardsDescription.trim()) {
          toast.error("Cards section description is required if heading exists");
          setLoading(false);
          return;
        }
      ...
      const res = await postAPI("/api/affiliate/create", submissionData, {
        headers: { "Content-Type": "multipart/form-data" },
      });

      if (res.data.data) {
        toast.success(res.data.message || "Affiliate page created successfully!");
        navigate("/super-admin/affiliate", { state: { reload: true } });
      } else {
            ...
            <div className="body">
              <form onSubmit={handleSubmit} encType="multipart/form-data">
                ...
                <div className="d-flex align-items-center mb-3" style={{ gap: "10px" }}>
                  <button type="submit" className="btn btn-primary" disabled={loading}>{loading ? "Creating..." : "Create Affiliate Page"}</button>
                </div>
              </form>
            </div>
          </div>
