...
const BlogTable = () => {
  ...
  const [deleteType] = useState("blogCMS");

  const fetchPages = async () => {
    try {
      const res = await getAPI("/api/blog-CMS");
      const data = Array.isArray(res.data.data) ? res.data.data : [];
      setPages(data);
    } catch (error) {
      console.error("Error fetching Blog CMS pages:", error);
      toast.error(error.response?.data?.message || "Failed to fetch blog pages");
      setPages([]);
    }
  };
  ...
  const handleDeleteConfirmed = async (id) => {
    try {
      await axiosInstance.delete(`/api/blog-CMS/delete/${id}`);
      ...
      <div className="block-header">
        <div className="row">
          <div className="col-lg-6 col-md-6 col-sm-12">
            <h2>Blog CMS Pages</h2>
          </div>
          <div className="col-lg-6 col-md-6 col-sm-12">
            <div className="d-flex flex-row-reverse">
              <button
                className="btn btn-secondary"
                onClick={() => navigate("/super-admin/CMS-Blog/create")}
              >
                      ...
                      <tr key={page._id}>
                        ...
                        <td>
                          <button
                            ...
                            onClick={() =>
                              navigate("/super-admin/CMS-Blog/update", {
                                state: { page },
                              })
                            }
                          >
