...
//       const sortedApplications = response.data.data.sort(
...
const ChallengesEntries = () => {
  ...
  // Fetch applications and sort latest first
  const fetchChallengeApplications = async () => {
    try {
      const response = await getAPI("/api/challenges-applications");
  ...
  const filteredApplications = applications.filter((app) =>
    ...
    (app.contactNumber || "").toLowerCase().includes(searchTerm.toLowerCase()) ||
    (app.category || "").toLowerCase().includes(searchTerm.toLowerCase()) ||
    (app.challenge || "").toLowerCase().includes(searchTerm.toLowerCase())
  );
        ...
        <div className="row">
          <div className="col-lg-6 col-md-6 col-sm-12">
            ...
            <ul className="breadcrumb">
              ...
              <li className="breadcrumb-item">
                <span
                  onClick={() => navigate("/super-admin/challenges")}
                  style={{ cursor: "pointer" }}
                >
                  Challenges
                </span>
        ...
        <div className="col-lg-12">
          <div className="card">
            ...
            <div className="body">
                  ...
                  <tbody>
                    {displayedApplications.length > 0 ? (
                      displayedApplications.map((app, index) => (
                        <tr key={app._id}>
                          <td>{(currentPage - 1) * applicationsPerPage + index + 1}</td>
                          <td>{app.fullName || '_'}</td>
                          <td>{app.email || '_'}</td>
                          <td>{app.contactNumber || '_'}</td>
                          <td>{app.challenge || '_'}</td>
                          <td>
                            <button
                              className="btn btn-outline-primary btn-sm mr-2"
                              title="View"
                              onClick={() =>
                                navigate('/super-admin/challenges/view-application', { state: { application: app } })
                              }
                            >
                              <i className="fa fa-eye"></i>
                            </button>
              ...
              <div className="pagination d-flex justify-content-between mt-4">
                <span className="d-none d-sm-inline-block text-truncate w-100 mx-1">
                  Showing {(filteredApplications.length === 0 ? 0 : (currentPage - 1) * applicationsPerPage + 1)} to{' '}
                  {Math.min(currentPage * applicationsPerPage, filteredApplications.length)} of {filteredApplications.length} entries
                </span>
                <ul className="pagination d-flex justify-content-end w-100">
                  <li className={`paginate_button page-item ${currentPage === 1 ? "disabled" : ""}`} onClick={handlePreviousPage}>
                    <button className="page-link">Previous</button>
                  </li>
                  ...
                  <li className={`paginate_button page-item ${currentPage === totalPages ? "disabled" : ""}`} onClick={handleNextPage}>
                    <button className="page-link">Next</button>
                  </li>
                </ul>
              </div>
