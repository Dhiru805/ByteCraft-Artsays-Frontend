...
function HowToBidCreate() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await postAPI("/api/how-to-bid/create", submissionData, {
        headers: { "Content-Type": "multipart/form-data" },
      });

      if (res.data.data) {
        toast.success(res.data.message || "How to Bid page created successfully!");
        navigate("/super-admin/how-to-bid", { state: { reload: true } });
      } else {
        toast.error(res.data.message || "Failed to create How to Bid page.");
      }
    } catch (err) {
              ...
              <form onSubmit={handleSubmit} encType="multipart/form-data">
                ...
                <div className="d-flex align-items-center mb-3" style={{ gap: "10px" }}>
                  ...
                  <button type="submit" className="btn btn-primary" disabled={loading}>
                    {loading ? "Creating..." : "Create How to Bid Page"}
                  </button>
                </div>
              </form>
            </div>
