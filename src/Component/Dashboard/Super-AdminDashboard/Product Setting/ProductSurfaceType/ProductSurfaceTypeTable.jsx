...
function ProductSurfaceTypeTable({ productSurfaceTypes, setProductSurfaceTypes, refreshProductSurfaceTypes }) {
  ...
  const ImportModal = ({ isOpen, onClose }) => {
    ...
    const handleImport = async () => {
      ...
      try {
        const res = await fetch("/api/import-productsurfacetype", { method: "POST", body: formData, headers: { "X-Requested-With": "XMLHttpRequest" } });
        if (!res.ok) throw new Error();
        const data = await res.json();
        toast.success(data.message || "Imported successfully!");
        await refreshProductSurfaceTypes();
        onClose();
      } catch { toast.error("Import failed. Please check your file."); }
    };
    const downloadTemplate = async () => {
      try {
        const res = await fetch("/api/export-productsurfacetype-template");
        if (!res.ok) throw new Error();
        const blob = await res.blob();
        const url = window.URL.createObjectURL(blob);
        const a = document.createElement("a"); a.href = url; a.download = "Product_Surface_Type_Template.xlsx";
    ...
    return (
      <Modal show={isOpen} onHide={onClose} centered>
        <div className="p-4">
          <h5 className="mb-3">Import Product Surface Types</h5>
          <div className="mb-3">
  ...
  const handleDeleteCancel = () => { setIsDeleteDialogOpen(false); setSelectedProductSurfaceTypeToDelete(null); };
  const handleDeleteConfirmed = (id) => { setProductSurfaceTypes(prev => prev.filter(t => t._id !== id)); setIsDeleteDialogOpen(false); };
  const openDeleteDialog = (t) => { setSelectedProductSurfaceTypeToDelete(t); setIsDeleteDialogOpen(true); };
  const openEditModal = (t) => { setSelectedProductSurfaceType(t); setIsProductSurfaceTypeModalOpen(true); };
  const handlePrevious = () => { if (currentPage > 1) setCurrentPage(currentPage - 1); };
  const handleNext = () => { if (currentPage < totalPages) setCurrentPage(currentPage + 1); };
  const handleProductsPerPageChange = (e) => { setProductsPerPage(Number(e.target.value)); setCurrentPage(1); };

  const filteredProductSurfaceTypes = productSurfaceTypes.filter(t => t.name?.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  return (
    <>
              ...
              <div className="table-responsive">
                <table className="table table-hover text-nowrap js-basic-example dataTable table-custom m-b-0 c_list">
                  <thead className="thead-dark">
                    <tr><th>#</th><th>Product Surface Type</th><th>Date</th><th>Action</th></tr>
                  </thead>
                  <tbody>
                    {currentProductSurfaceTypes.length === 0 ? (
                      ...
                      currentProductSurfaceTypes.map((productSurfaceType, index) => (
                        <tr key={productSurfaceType._id}>
                          ...
                          <td>
                            <button type="button" className="btn btn-outline-info btn-sm mr-2" title="Edit" onClick={() => openEditModal(productSurfaceType)}><i className="fa fa-pencil"></i></button>
                            <button type="button" className="btn btn-outline-danger btn-sm mr-2" title="Delete" onClick={() => openDeleteDialog(productSurfaceType)}><i className="fa fa-trash-o"></i></button>
                          </td>
                        </tr>
                      ))
                    )}
      ...
      {isDeleteDialogOpen && <ConfirmationDialog onClose={handleDeleteCancel} deleteType="productsurfacetype" id={selectedProductSurfaceTypeToDelete._id} onDeleted={handleDeleteConfirmed} />}
      {isProductSurfaceTypeModalOpen && <ProductSurfaceTypeModal onClose={() => setIsProductSurfaceTypeModalOpen(false)} refreshProductSurfaceTypes={refreshProductSurfaceTypes} selectedProductSurfaceType={selectedProductSurfaceType} />}
