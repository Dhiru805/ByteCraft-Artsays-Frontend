...
function PeriodEraTable({ periodEras, setPeriodEras, refreshPeriodEras }) {
  ...
  const handleDeleteCancel = () => {
    ...
    setSelectedPeriodEraToDelete(null);
  };

  const handleDeleteConfirmed = (id) => {
    setPeriodEras((prevPeriodEras) =>
      prevPeriodEras.filter((periodEra) => periodEra._id !== id)
    );
    setIsDeleteDialogOpen(false);
  };

  const openDeleteDialog = (periodEra) => {
    setSelectedPeriodEraToDelete(periodEra);
    setIsDeleteDialogOpen(true);
  };

  const openEditModal = (periodEra) => {
    setSelectedPeriodEra(periodEra);
    setIsPeriodEraModalOpen(true);
  };
  ...
  const filteredPeriodEras = periodEras.filter(periodEra =>
    periodEra.name?.toLowerCase().includes(searchTerm.toLowerCase())
  );
    ...
    <>
                        ...
                        <tr key={periodEra._id}>
                          ...
                          <td>
                            <button
                              ...
                              title="Edit"
                              onClick={() => openEditModal(periodEra)}
                            >
                            ...
                            <button
                              ...
                              title="Delete"
                              onClick={() => openDeleteDialog(periodEra)}
                            >
      ...
      {isDeleteDialogOpen && (
        <ConfirmationDialog
          ...
          deleteType="periodera"
          id={selectedPeriodEraToDelete._id}
          onDeleted={handleDeleteConfirmed}
        />
      )}
      {isPeriodEraModalOpen && (
        <PeriodEraModal
          onClose={() => setIsPeriodEraModalOpen(false)}
          refreshPeriodEras={refreshPeriodEras}
          selectedPeriodEra={selectedPeriodEra}
        />
      )}
    </>
  );
