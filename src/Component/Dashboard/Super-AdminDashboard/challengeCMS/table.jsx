...
const ChallengeTable = () => {
  ...
  const [deleteType] = useState("challengeCMS");

  const fetchPages = async () => {
    try {
      const res = await getAPI("/api/challenge-CMS");
      const data = Array.isArray(res.data.data) ? res.data.data : [];
      setPages(data);
    } catch (error) {
      console.error("Error fetching Challenge CMS pages:", error);
      toast.error(error.response?.data?.message || "Failed to fetch challenge pages");
    }
  };
  ...
  const handleDeleteConfirmed = async (id) => {
    try {
      await axiosInstance.delete(`/api/challenge-CMS/delete/${id}`);
      ...
      <div className="block-header">
        <div className="row">
          <div className="col-lg-6">
            <h2>Challenge CMS Pages</h2>
          </div>
          <div className="col-lg-6">
            <div className="d-flex flex-row-reverse">
              <button
                className="btn btn-secondary"
                onClick={() => navigate("/super-admin/challenge-CMS/create")}
              >
                      ...
                      <tr key={page._id}>
                        ...
                        <td>
                          <button
                            ...
                            onClick={() =>
                              navigate("/super-admin/challenge-CMS/update", {
                                state: { page },
                              })
                            }
                          >
