...
const OrderView = () => {
  ...
  useEffect(() => {
    const fetchOrderDetails = async () => {
      try {
        ...
        const res = await getAPI(`/api/package-material/order/${id}`);
        ...
        if (res.data && res.data.data) {
          ...
          if (order.deliveryAddress) {
            let a = order.deliveryAddress.address;

            if (typeof a === "string") {
              try {
                a = JSON.parse(a);
              } catch (e) {
                console.warn("Could not parse deliveryAddress.address:", e);
                a = {}; 
              }
            }
            ...
            if (a && typeof a === "object" && a["0"]) {
              ...
              try {
                ...
                console.warn("Double-encoded address:", e);
              }
            }

            const addressFields = [
              a?.line1 || order.deliveryAddress.line1,
              a?.line2 || order.deliveryAddress.line2,
              a?.landmark || order.deliveryAddress.landmark,
              a?.city || order.deliveryAddress.city,
              a?.state || order.deliveryAddress.state,
              a?.country || order.deliveryAddress.country,
              a?.pincode || order.deliveryAddress.pincode,
            ];

            orderDeliveryAddress = addressFields.filter(Boolean).join(", ");
          } else {
            orderDeliveryAddress = "No Address Found";
          }
  ...
  useEffect(() => {
    const fetchOrderDetails = async () => {
      try {
        const userId = localStorage.getItem("userId");
        const res = await getAPI(`/api/package-material/seller/order/${id}`);
        console.log("Fetched seller order details:", res);
        if (res.data && res.data.data) {
          const order = res.data.data;
          ...
          <div className="col-lg-6 col-md-6 col-sm-12">
            ...
            <ul className="breadcrumb">
              ...
              <li className="breadcrumb-item active">
                <span
                  onClick={() => navigate("/super-admin/packaging-material/order")}
                  style={{ cursor: "pointer" }}
                >
