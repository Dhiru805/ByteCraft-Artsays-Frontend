...
const UpdateLicensing = () => {
  ...
  useEffect(() => {
    if (!page) {
      toast.error("No Licensing page data provided.");
      navigate("/super-admin/licensing");
    } else {
      setFormData({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await putAPI(`/api/licensing/update/${page._id}`, submissionData, {
        headers: { "Content-Type": "multipart/form-data" },
      });

      if (response.data.data) {
        toast.success("Licensing page updated successfully!");
        navigate("/super-admin/licensing", { state: { reload: true } });
      } else {
        toast.error(response.data.message || "Failed to update Licensing page");
      }
    } catch (error) {
