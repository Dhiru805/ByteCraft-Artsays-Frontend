  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      for (let i = 0; i < formData.articles.length; i++) {
        if (!formData.articles[i].bannerImage && !formData.articles[i].existingBanner) {
      ...
      formData.articles.forEach((article, idx) => {
        ...
        if (article.existingBanner)
          submissionData.append(`articles[${idx}][existingBanner]`, article.existingBanner);
        if (article.buttonName)
          submissionData.append(`articles[${idx}][buttonName]`, article.buttonName.trim());
        if (article.buttonPath)
          submissionData.append(`articles[${idx}][buttonPath]`, article.buttonPath.trim());
      });

      const res = await postAPI("/api/licensing/create", submissionData, {
        headers: { "Content-Type": "multipart/form-data" },
      });

      if (res.data.data) {
        toast.success(res.data.message || "Licensing page created successfully!");
        navigate("/super-admin/licensing", { state: { reload: true } });
      } else {
        toast.error(res.data.message || "Failed to create licensing page.");
      }
    } catch (err) {
              ...
              <form onSubmit={handleSubmit} encType="multipart/form-data">
                ...
                >
                  ...
                  <button
                    ...
                    disabled={loading}
                  >
                    {loading ? "Creating..." : "Create Licensing Page"}
                  </button>
                </div>
              </form>
            </div>
...
export default LicensingCreate;
