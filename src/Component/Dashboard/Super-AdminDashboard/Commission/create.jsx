...
function CommissionCreate() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await postAPI("/api/commission/create", submissionData, {
        headers: { "Content-Type": "multipart/form-data" },
      });

      if (res.data.data) {
        toast.success(res.data.message || "Commission page created successfully!");
        navigate("/super-admin/commission", { state: { reload: true } });
      } else {
        toast.error(res.data.message || "Failed to create commission page.");
      }
    } catch (err) {
              ...
              <form onSubmit={handleSubmit} encType="multipart/form-data">
                ...
                <div className="d-flex align-items-center mb-3" style={{ gap: "10px" }}>
                  ...
                  <button type="submit" className="btn btn-primary" disabled={loading}>
                    {loading ? "Creating..." : "Create Commission Page"}
                  </button>
                </div>
              </form>
            </div>
