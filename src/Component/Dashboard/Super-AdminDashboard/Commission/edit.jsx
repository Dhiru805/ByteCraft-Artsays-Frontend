...
const UpdateCommission = () => {
  ...
  useEffect(() => {
    if (!page) {
      toast.error("No Commission page data provided.");
      navigate("/super-admin/commission");
    } else {
      setFormData({
      ...
      setBannerPreviews(
        page.articles?.map((a) =>
          a.bannerImage || a.existingBanner
            ? getImageUrl(a.bannerImage || a.existingBanner)
            : null
        ) || [null]
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await putAPI(`/api/commission/update/${page._id}`, submissionData, {
        headers: { "Content-Type": "multipart/form-data" },
      });

      if (response.data.data) {
        toast.success("Commission page updated successfully!");
        navigate("/super-admin/commission", { state: { reload: true } });
      } else {
        toast.error(response.data.message || "Failed to update Commission page");
      }
    } catch (error) {
