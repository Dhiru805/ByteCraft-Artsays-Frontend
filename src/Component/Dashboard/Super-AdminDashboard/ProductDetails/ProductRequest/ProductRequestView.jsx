...
function AllProduct() {
    ...
    useEffect(() => {
        const fetchProduct = async () => {
            try {
                const result = await getAPI(`${process.env.REACT_APP_API_URL}/api/getallartistbuyerprodyctdetailsbyid/${productId}`, {}, true, false);
                                        ...
                                        {product.userId && (
                                            <>
                                                <button className="btn btn-outline-dark ms-2"
                                                    onClick={() =>   navigate(`/${userType}/Dashboard/productrequest/productdetails/editproduct/${product._id}`)}>Edit</button>
