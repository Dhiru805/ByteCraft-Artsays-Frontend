...
function ProductUpload() {
  ...
  const handleSubmit = async (e) => {
    ...
    // if (formData.additionalMedia && formData.additionalMedia.length > 3) {
    //   toast.error("Maximum 3 additional media files allowed");
    //   setIsSubmitting(false);
    //   return;
    // }

    // if (formData.provenanceDocument && formData.provenanceDocument.length > 1) {
    //   toast.error("Only one provenance document allowed");
    //   setIsSubmitting(false);
    //   return;
    // }
    ...
    try {
      ...
      formDataToSend.append('dimensions[width]', parseFloat(formData.width));
      formDataToSend.append('dimensions[height]', parseFloat(formData.height));
      formDataToSend.append('dimensions[depth]', parseFloat(formData.depth));
      if (formData.weight) {
      ...
      //   formData.additionalMedia.forEach((file) => {
      //     formDataToSend.append('additionalMedia', file);
      //   });
      // }
      // if (formData.provenanceDocument) {
      //   formDataToSend.append('provenanceDocument', formData.provenanceDocument);
      // }
  ...
  for (let [key, value] of formDataToSend.entries()) {
    console.log(`${key}: ${value instanceof File ? value.name : value}`);
      ...
      const response = await postAPI('/api/addProduct', formDataToSend, {
        headers: {
  ...
  const tabValidators = {
    ...
    payoutDetails: () => {
      const missingFields = [];
      if (formData.giftWrapping && formData.giftWrappingCost) {
        if (!formData.giftWrappingCostAmount || formData.giftWrappingCostAmount <= 0) {
          missingFields.push("Gift Wrapping Cost Amount (must be greater than 0)");
        }
      }
      if (missingFields.length > 0) {
