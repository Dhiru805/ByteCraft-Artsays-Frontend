...
const WhyFromArtsaysCreate = () => {
        ...
        if (sectionRes.data.success && sectionRes.data.data) {
          ...
          setFormData({
            ...
            cards: section.cards?.length ? section.cards.map(card => {
              // Normalize icon path for URL
              const normalizedIcon = card.icon ? card.icon.replace(/\\/g, '/') : null;
              return {
                image: null,
  ...
  const addCard = () => {
    setFormData({
      ...formData,
      cards: [...formData.cards, { image: null, icon: null, preview: null, title: "", description: "" }],
    });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      for (let idx = 0; idx < formData.cards.length; idx++) {
        ...
        if (!c.image && !c.icon) {
          toast.error(`Card ${idx + 1} icon is required`);
          setLoading(false);
          return;
        }
        ...
        if (c.image) {
          ...
          // If no new image, send the existing icon path back
              ...
              <form onSubmit={handleSubmit} encType="multipart/form-data">
                ...
                {formData.cards.map((c, idx) => (
                  <div key={idx} className="border mb-3 p-3 rounded shadow">
                    <div className="form-group">
                      <label>Card Image {!(c.preview || c.image) && "*"}</label>
                      <input
                        type="file"
                        accept="image/jpeg,image/png"
                        onChange={(e) => handleChange(e, idx, "image")}
                        className="form-control"
                        required={!(c.preview || c.image)}
                      />
                      {c.preview && (
                        <img
                          src={c.preview}
