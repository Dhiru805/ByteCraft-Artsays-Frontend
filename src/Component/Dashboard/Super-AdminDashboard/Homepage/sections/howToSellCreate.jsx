...
const HowToSellCreate = () => {
  ...
  useEffect(() => {
    const loadHomepageAndSection = async () => {
      try {
        ...
        const secRes = await getAPI(`/api/homepage-sections/how-to-sell/${page._id}`);
        if (secRes.data?.success && secRes.data?.data) {
          ...
          setExistingCardImages(
            (s.cards || []).map(c =>
              c.image ? getImageUrl(c.image.replace(/\\/g, "/")) : null
            )
          );
          ...
          setExistingIcons(
            (s.cards || []).map(c =>
              (c.icons || []).map(icon =>
                icon ? getImageUrl(icon.replace(/\\/g, "/")) : null
              )
            )
          );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await postAPI("/api/homepage-sections/how-to-sell/create", submissionData, {
        headers: { "Content-Type": "multipart/form-data" },
      });

      if (res.data.success) {
        toast.success(res.data.message || "How To Sell section created successfully!");
  ...
  return (
    <div className="container-fluid">
      <div className="block-header">
        <h2>Create How To Sell Section</h2>
...
export default HowToSellCreate;