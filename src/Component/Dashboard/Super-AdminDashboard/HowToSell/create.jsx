...
function HowToSellAdmin() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      //   if (!article.articleHeading.trim() || !article.articleContent.trim()) {
      //     toast.error(`All fields are required for article ${i + 1}`);
      //     setLoading(false);
      //     return;
      //   }
      // }
      ...
      const res = await postAPI("/api/howtosell/create", submissionData, {
        headers: { "Content-Type": "multipart/form-data" },
      });

      if (res.data.data) {
        toast.success(res.data.message || "Page created successfully!");
        navigate("/super-admin/how-to-sell", { state: { reload: true } });
      } else {
