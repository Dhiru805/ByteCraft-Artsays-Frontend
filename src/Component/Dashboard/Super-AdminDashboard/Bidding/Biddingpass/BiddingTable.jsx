...
const BiddingTable = () => {
    ...
    const togglePassActive = async (passId, active) => {
        try {
            const res = await putAPI(`/api/bidding/passes/${passId}/status`, { active: !active }, {}, true);
            if (!res?.hasError) {
                        ...
                        <div className="body">
                                ...
                                <table className="table table-hover text-nowrap">
                                    ...
                                    <tbody>
                                        {paginatedPasses.length === 0 ? (
                                            <tr>
                                                <td colSpan="10" className="text-center">No passes</td>
                                            </tr>
                                        ) : (
                                            paginatedPasses.map((pass, idx) => (
                                                <tr key={pass._id || idx}>
                                                    ...
                                                    <td>{pass.pricing}</td>
                                                    <td>
                                                        <span className={`badge ${pass.active ? 'badge-success' : 'badge-secondary'}`}>
                                                            {pass.active ? 'Active' : 'Inactive'}
                                                        </span>
                                                    </td>
                                                    <td>
                                                        <button
                                                            className={`btn btn-sm ${pass.active ? 'btn-warning' : 'btn-success'} mr-2`}
                                                            onClick={() => togglePassActive(pass._id, pass.active)}
                                                        >
                                                            {pass.active ? 'Deactivate' : 'Activate'}
                                                        </button>
                                                        <button className="btn btn-sm btn-danger" onClick={() => deletePass(pass._id)}>
                                                            Delete
                                                        </button>
                                                    </td>
                                                </tr>
                            ...
                            <div className="pagination d-flex justify-content-between mt-4">
                                <span className="mx-1 d-none d-sm-inline-block text-truncate w-100">
                                    Showing {(currentPage - 1) * itemsPerPage + 1} to {Math.min(currentPage * itemsPerPage, filteredPasses.length)} of {filteredPasses.length} entries
                                </span>
