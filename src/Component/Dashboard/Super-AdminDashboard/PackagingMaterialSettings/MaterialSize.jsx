...
const MaterialSize = () => {
  ...
  const filteredItems = materials.filter(mat => mat.materialSize?.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  const fetchMaterials = async () => {
    try {
      ...
      if (res.data.success) {
        setMaterials(Array.isArray(res.data.materialSize) ? res.data.materialSize : [res.data.materialSize]);
      }
    } catch (error) {
  ...
  const openEditModal = (mat) => {
    setEditMode(true);
    setMaterialSize(mat.materialSize);
    setEditId(mat._id);
    setShowPopUp(true);
  }
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await postAPI("/api/packaging-material-setting/material-size/create",
        {
  ...
  const handleUpdate = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await putAPI(`/api/packaging-material-setting/material-size/update/${editId}`,
        {
          userId: localStorage.getItem("userId"),
          materialSize
        }
          ...
          <div className="col-lg-6 col-md-6 col-sm-12">
            <div className="d-flex flex-row-reverse">
              <div className="page_action">
                ...
                >
                  <i className="fa fa-plus"></i> Create Material Size
                </button>
                {showPopUp && (
                  <div
                    ...
                    <div className="modal-dialog">
                      <div className="modal-content">
                        <div className="modal-header">
                          <h5 className="modal-title">{viewMode ? "View Material Size" : editMode ? "Update Material Size" : "Create Material Size"}</h5>
                          <button
                            type="button"
                            className="close"
                            ...
                            <div className="modal-footer">
                              ...
                              >
                                {loading
                                  ...
                                  editMode ? "Update Material Size" : "Create Material Size"}
                              </button>
                            </div>
                          </form>
                          )}
            ...
            <div className="body">
              <div className="table-responsive">
                <table className="table table-hover">
                  ...
                  <tbody>
                    {materials.filter((mat) => mat.materialSize.toLowerCase().includes(searchTerm.toLowerCase())).length === 0 ? (
                      <tr>
                      ...
                      materials.filter((mat) => mat.materialSize.toLowerCase().includes(searchTerm.toLowerCase())).map((mat, index) => (
                        <tr key={mat._id}>
                          <td>{(currentPage - 1) * perPage + index + 1}</td>
                          <td>{mat.materialSize}</td>
                          <td>
                            <button
                              type="button"
