...
const AddressModal = ({ isOpen, onClose, userId, fetchProfile }) => {
  ...
  const fetchDefaultAddress = async () => {
    ...
    setLoading(true);
    try {
      const response = await getAPI(`/api/get-default-address/${userId}`, {}, true, false);
      if (!response.hasError && response.data?.data) {
        setDefaultAddressId(response.data.data.addressId);
        setError('');
      } else {
...
const checkPincodeAvailability = async () => {
  ...
  try {
    ...
    const { isServiceable, city, state } = response.data.data;
  ...
  const handleInputChange = (e) => {
    ...
    if (name === 'pincode') {
      setPincodeStatus({ checked: false, isServiceable: false, message: '', loading: false });
      setLastCheckedPincode('');
    }
  };
          ...
          <form onSubmit={handleCreateAddress}>
            ...
            <div className="row">
              ...
              <div className="col-md-6 mb-3">
                ...
                {pincodeStatus.checked && (
                  <small className={pincodeStatus.isServiceable ? "text-success" : "text-danger"}>
                    {pincodeStatus.message}
                  </small>
                )}
              </div>
