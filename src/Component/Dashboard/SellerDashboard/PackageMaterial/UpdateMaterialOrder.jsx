...
const UpdateMaterialOrder = () => {
   ...
   const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await putAPI(
        `/api/package-material/seller/order/update/${id}`,
        data
      );
 ...
 const handleDropdownChange = async (type, id) => {
  ...
  const handleQuantityChange = (e) => {
    ...
    setQuantities((prev) => {
      const updatedQty = (prev[type] || 0) + qty;
      const updatedQuantities = { ...prev, [type]: updatedQty };

      const total = Object.keys(prices).reduce((acc, key) => {
      const price = prices[key] || 0;
      const quantity = updatedQuantities[key] || 0;
...
export default UpdateMaterialOrder;