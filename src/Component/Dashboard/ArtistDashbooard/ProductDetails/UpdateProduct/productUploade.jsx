...
function ProductUpload() {
  ...
  const prepareFormData = (status = "Pending") => {
    ...
    formData.materials.forEach((m) =>
      fd.append("materials", m.value || m.label)
    );
    ...
    if (formData.surfaceType)
      fd.append("surfaceType", formData.surfaceType?.value || "");
    if (formData.culturalRegion)
      fd.append("culturalRegion", formData.culturalRegion?.value || "");
    if (formData.biologicalMaterial)
      fd.append("biologicalMaterial", formData.biologicalMaterial);
    fd.append("functionalUse", formData.functionalUse?.value || "");
    if (formData.materialSource)
      fd.append("materialSource", formData.materialSource);
    if (formData.craftTechnique)
      fd.append("craftTechnique", formData.craftTechnique);
    ...
    if (pricingData.marketPrice)
      fd.append("marketPrice", parseFloat(pricingData.marketPrice));
    if (pricingData.discount)
      fd.append("discount", parseFloat(pricingData.discount));
    ...
    if (formData.blockchainNetwork)
      fd.append("blockchainNetwork", formData.blockchainNetwork.value);
    fd.append("smartContractAddress", formData.smartContractAddress || "");
    if (formData.tokenStandard)
      fd.append("tokenStandard", formData.tokenStandard.value);
    ...
    if (formData.mintingType)
      fd.append("mintingType", formData.mintingType.value);
    if (formData.licenseType)
      fd.append("licenseType", formData.licenseType.value);
    ...
    if (formData.unlockableContent) {
      fd.append(
        ...
        formData.unlockableContentLink?.trim() || ""
      );
    }
    ...
    if (formData.partOfCollection)
      fd.append("editionSize", parseInt(formData.editionSize) || 1);
    ...
    if (formData.originRegion)
      fd.append("originRegion", formData.originRegion.value);
    if (formData.periodEra) fd.append("periodEra", formData.periodEra.value);
    if (formData.antiqueCondition)
      fd.append("antiqueCondition", formData.antiqueCondition.value);
    if (formData.conservationStatus)
      fd.append("conservationStatus", formData.conservationStatus.value);
    ...
    if (formData.originalReproduction)
      fd.append("originalReproduction", formData.originalReproduction.value);
    fd.append(
      "museumExhibitionHistory",
      formData.museumExhibitionHistory || ""
    );
    if (formData.maintenanceRequired)
      fd.append("maintenanceRequired", formData.maintenanceRequired.value);
    fd.append(
      "customEngravingAvailable",
      formData.customEngravingAvailable || false
    );
    ...
    fd.append(
      "addressLine1",
      formData.addressLine1 || profileData.address?.line1 || ""
    );
    fd.append(
      "addressLine2",
      formData.addressLine2 || profileData.address?.line2 || ""
    );
    fd.append(
      "landmark",
      formData.landmark || profileData.address?.landmark || ""
    );
    fd.append("city", formData.city || profileData.address?.city || "");
    fd.append("state", formData.state || profileData.address?.state || "");
    fd.append(
      "country",
      formData.country || profileData.address?.country || ""
    );
    fd.append(
      "pincode",
      formData.pincode || profileData.address?.pincode || ""
    );
...
const handleAddressSaved = () => {
  ...
  const isNFTArtSelected =
    formData.category?.label === "NFT Art" ||
    formData.subCategory?.label === "NFT Art";

  const isAntiqueVintageSelected =
    formData.mainCategory?.label === "Antiques & Vintage";
  ...
  const tabOrder = [
    "basic",
    ...(isNFTArtSelected ? ["nft"] : []),
    ...(isAntiqueVintageSelected ? ["antique"] : []),
    "images",
    "artwork",
    "pricing",
    "shipping",
  ...
  const tabValidators = {
    ...
    artwork: () => {
      ...
      if (
        (medium?.value?.toLowerCase() === "print" ||
          medium?.value?.toLowerCase() === "poster") &&
    ...
    pricing: () => {
      ...
      if (
        includeGst &&
        (!gstPercentage || gstPercentage <= 0 || gstPercentage > 100)
      ) {
    ...
    payoutDetails: () => {
      ...
      if (giftWrappingCost) {
        if (
          !giftWrappingCostAmount ||
          isNaN(giftWrappingCostAmount) ||
          Number(giftWrappingCostAmount) <= 0
        ) {
    ...
    antique: () => {
      ...
      if (!formData.originalReproduction)
        missingFields.push("Original vs. Reproduction");
      ...
      if (formData.conservationStatus?.value === "restored") {
        if (!formData.restorationHistory)
          missingFields.push("Restoration History");
        if (!formData.restorationDocumentation)
          missingFields.push("Restoration Documentation");
      }

      if (!formData.conservationStatus)
        missingFields.push("Conservation Status");
      if (!formData.certification) missingFields.push("Certification Document");
      if (!formData.maintenanceRequired)
        missingFields.push("Maintenance Required");
      ...
      if (missingFields.length > 0) {
        toast.error(
          `Please fill Required Details: ${missingFields.join(", ")}.`
        );
        return false;
      }
              ...
              <ul className="nav nav-tabs mb-4">
                ...
                <li className="nav-item">
                  <button
                    type="button"
                    className={`nav-link ${
                      activeTab === "payoutDetails" ? "active" : ""
                    }`}
