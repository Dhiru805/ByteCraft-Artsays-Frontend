...
const TYPE_ICON = {
  ...
  verification_update_required:      "⚠️",
  // Dashboard / Smart Alerts
  no_products_listed:                "📦",
  products_live_no_sales:            "📊",
  high_views_low_conversion:         "👁️",
  low_wallet_balance:                "💰",
  pending_withdrawals:               "⏳",
  certification_recommended:         "🏅",
  insurance_recommended:             "🛡️",
  exhibition_invite:                 "🖼️",
  featured_artist_announcement:      "⭐",
  ...
  custom_order_price_sent:           "💬",
  buyer_accepted_custom_price:       "✅",
  buyer_rejected_custom_price:       "❌",
  custom_order_payment_received:     "💳",
  custom_order_deadline_reminder:    "⏰",
  custom_order_marked_complete:      "✅",
  custom_order_delivered:            "🚚",
  // Bidding
  product_added_to_bidding:          "🔨",
  auction_approved:                  "✅",
  bidding_product_updated:           "✏️",
  first_bid_received:                "🎯",
  new_highest_bid:                   "📈",
  auction_ending_soon:               "⏰",
  auction_won:                       "🏆",
  auction_completed:                 "🏆",
  auction_restarted:                 "🔄",
  bidding_pass_purchased:            "🎟️",
  bidding_pass_upgraded:             "⬆️",
  bidding_pass_assigned:             "🔗",
  ...
  blog_rejected:                     "❌",
  blog_published:                    "🌐",
  blog_performance_milestone:        "📈",
  // Certification
  certification_request_submitted:   "📋",
  certification_payment_successful:  "💳",
  certification_payment_failed:      "❌",
  certification_approved:            "🏅",
  certification_rejected:            "❌",
  certification_expiring_soon:       "⏳",
  // Insurance
  insurance_purchased:               "🛡️",
  insurance_approved:                "✅",
  insurance_expiring_soon:           "⏳",
  insurance_claim_eligible:          "📋",
  // Exhibition
  exhibition_application_submitted:  "🖼️",
  exhibition_approved:               "✅",
  exhibition_rejected:               "❌",
  ...
  ad_campaign_live:                  "🟢",
  ad_campaign_paused:                "⏸️",
  ad_campaign_ended:                 "🏁",
  sponsored_product_alert:           "📊",
  promotion_performance_alert:       "📈",
  // Badges & Membership
  premium_badge_purchased:           "⭐",
  premium_badge_activated:           "✅",
  premium_badge_expiring_soon:       "⏳",
  membership_purchased:              "👑",
  membership_expired:                "⏳",
  // Packaging
  packaging_order_placed:            "📦",
  packaging_payment_successful:      "💳",
  packaging_order_updated:           "✏️",
  packaging_order_delivered:         "✅",
  // Wallet
  wallet_credited:                   "💚",
  wallet_debited:                    "🔴",
  transaction_recorded:              "📋",
  wallet_topup_successful:           "💳",
  wallet_topup_failed:               "❌",
...
const ArtistNotificationsPage = () => {
  ...
  const fetchNotifications = useCallback(async (pg = 1, cat = category, f = filter) => {
    ...
    try {
      const res = await getAPI(
        `/api/artist-notifications/${userId}?page=${pg}&limit=${LIMIT}&category=${cat}&filter=${f}`
      );
      if (res?.data?.success) {
  ...
  const handleMarkRead = async (notifId) => {
    try {
      await putAPI(`/api/artist-notifications/${userId}/mark-read`, { notificationId: notifId });
      setNotifications((prev) => prev.map((n) => n._id === notifId ? { ...n, isRead: true } : n));
      setUnreadCount((c) => Math.max(0, c - 1));
      window.dispatchEvent(new CustomEvent("artistNotifUpdated", { detail: { notificationId: notifId } }));
    } catch (_) {}
  };

  const handleMarkAllRead = async () => {
    try {
      await putAPI(`/api/artist-notifications/${userId}/mark-read`, { all: true });
      setNotifications((prev) => prev.map((n) => ({ ...n, isRead: true })));
      setUnreadCount(0);
      window.dispatchEvent(new CustomEvent("artistNotifUpdated", { detail: { all: true } }));
    } catch (_) {}
  };
  ...
  const handleDelete = async (notifId) => {
    ...
    try {
      await deleteAPI(`/api/artist-notifications/${userId}/${notifId}`);
  ...
  const handleNotificationClick = async (n) => {
    if (!n.isRead) {
      try {
        await putAPI(`/api/artist-notifications/${userId}/mark-read`, { notificationId: n._id });
        setNotifications((prev) => prev.map((item) => item._id === n._id ? { ...item, isRead: true } : item));
        setUnreadCount((c) => Math.max(0, c - 1));
        window.dispatchEvent(new CustomEvent("artistNotifUpdated", { detail: { notificationId: n._id } }));
      } catch (_) {}
    }
